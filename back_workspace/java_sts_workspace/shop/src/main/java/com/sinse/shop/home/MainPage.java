...
public class MainPage extends Page{
	...
	public void createRecentList() {
		List<Product> productList = productDAO.slectRecentList(3);
		
		for(int i=0;i<productList.size();i++) {
			Product product = productList.get(i);
			for(String s: product.getFilenameList()) {
				System.out.println(i+" filename is " + s);
			}
			
			ProductItem productItem = new ProductItem(this, product);
